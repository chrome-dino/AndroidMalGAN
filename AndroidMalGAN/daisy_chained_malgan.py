...
def daisy_chain_attack(file_path='', n_count=5):
    f_list = list(os.path.split(file_path))
    f_name = f_list[-1]
    f_list[-1] = 'modified_' + f_name
    file_path_mod = os.path.join(*f_list)
    # inject ngram
    ngrams_inject(file_path, copy_file=True, n_count=n_count)
    # inject permissions
    permission_inject(file_path_mod, copy_file=False)
    # inject intent
    intent_inject(file_path_mod, copy_file=False)
    # inject api
    api_inject(file_path_mod, copy_file=False)
