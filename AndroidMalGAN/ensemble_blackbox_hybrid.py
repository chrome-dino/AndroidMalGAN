...
def validate_ensemble(generator, bb_name, model_name, data_malware, data_benign):
    ...
    acc_ben = ben / (ben + mal)
    print(
        f'test set benign predicted: {str(ben)} benign files and {str(mal)} malicious files on ensemble model against {model_name} malgan trained on {bb_name} | {str(acc_ben * 100)}% score')
    ...
    acc_mal = mal / (ben + mal)
    print(
        f'test set malware predicted: {str(ben)} benign files and {str(mal)} malicious files on ensemble model against {model_name} malgan trained on {bb_name} | {str(acc_mal * 100)}% score')
