...
logging.basicConfig(filename=datetime.now().strftime('permissions_%H_%M_%d_%m_%Y.log'), encoding='utf-8', level=logging.INFO)
...
BB_MODELS = [{'name': 'rf', 'path': '../rf_permissions_model.pth'}, {'name': 'dt', 'path': '../dt_permissions_model.pth'},
             ...
             {'name': 'mlp', 'path': '../permissions_mlp.pth'}]
...
MALWARE_CSV = '/home/dsu/Documents/AndroidMalGAN/malware_permissions.csv'
BENIGN_CSV = '/home/dsu/Documents/AndroidMalGAN/benign_permissions.csv'
# BB_SAVED_MODEL_PATH = 'opcode_ngram_blackbox.pth'

os.environ['TUNE_DISABLE_STRICT_METRIC_CHECKING'] = '1'


def train_permissions_model(config, blackbox=None, bb_name=''):
    ...
    discriminator, generator, lossfun, disc_optimizer, gen_optimizer = create_permissions_model(config['lr_gen'],
                                                                                                 config[
                                                                                                     'l2_lambda_gen'],
                                                                                                 config['lr_disc'],
                                                                                                 config[
    ...
    if not RAY_TUNE:
        ...
        plt.savefig(
            os.path.join('/home/dsu/Documents/AndroidMalGAN/results_permissions', f'permissions_' + bb_name + '_gen_loss.png'),
            bbox_inches='tight')
        plt.clf()
        ...
        # plt.legend(['Discrimator', 'Generator'])
        plt.savefig(
            os.path.join('/home/dsu/Documents/AndroidMalGAN/results_permissions',
        ...
        plt.title(f'Permissions Model Accuracy Train ({str(bb_name)})')
        plt.savefig(
            os.path.join('/home/dsu/Documents/AndroidMalGAN/results_permissions',
                         f'permissions_' + bb_name + '_model_acc_train.png'),
            bbox_inches='tight')
        plt.clf()
    ...
    if not RAY_TUNE:
        ...
        validate_ensemble(generator, bb_name, 'permissions', test_data_malware, test_data_benign)
...
def create_permissions_model(learning_rate_gen, l2lambda_gen, learning_rate_disc, l2lambda_disc, classifier,
    ...
    discriminator = PermissionsClassifier(l2=classifier['l1'], l3=classifier['l2'],
                                    l4=classifier['l3'])
    generator = PermissionsGenerator(l2=generator['l1'], l3=generator['l2'],
...
class PermissionsClassifier(nn.Module):
    def __init__(self, d_input_dim=350, l2=700, l3=450, l4=325):
        super(PermissionsClassifier, self).__init__()
...
class PermissionsGenerator(nn.Module):
    def __init__(self, noise_dims=70, input_layers=350, l2=840, l3=1480, l4=740):
        super(PermissionsGenerator, self).__init__()
...
def validate(generator, blackbox, bb_name, data_malware, data_benign):
    ...
    if os.path.isfile(f'results_permissions.csv'):
        ...
        df.to_csv(f'results_permissions.csv', mode='a', header=False)
    else:
        df = pd.DataFrame([results])
        df.to_csv(f'results_permissions.csv')
...
def train():
    ...
    if os.path.exists('blackbox_crosscheck_permission.txt'):
        os.remove('blackbox_crosscheck_permission.txt')
    ...
    for bb_model in BB_MODELS:
        ...
        if RAY_TUNE:
            tune_config = {
                ...
                "g_1": tune.choice([650, 700, 750, 800, 850, 900, 950, 1000, 1050, 1100, 1150]),
                "g_2": tune.choice([1250, 1350, 1450, 1550, 1650, 1750, 1850, 1950, 2050, 2150, 2250]),
                "g_3": tune.choice([650, 700, 750, 800, 850, 900, 950, 1000, 1050, 1100, 1150]),
                "c_1": tune.choice([600, 550, 500, 475, 450, 425, 400, 375, 350, 325, 300]),
                "c_2": tune.choice([475, 450, 425, 400, 375, 350, 325, 300, 275]),
